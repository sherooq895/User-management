...
function Createuser() {
    ...
    const applicationForm = async (e) => {
       ...
       console.log(application,'dsdsdsds');
            ...
            <form autoComplete="off"  onSubmit={handleSubmit(applicationForm)} >
                <div className="grid-cols-1  w-full grid md:grid-cols-2 gap-2 p-5">
                    <div className='bg-gray-100 w-full p-2 flex items-start flex-col mb-5'>
                        <input type="text" {...register('name', { required: true, pattern: /^@?(\w){1,50}$/ })}   id="name"  onChange={handleChange} placeholder='Name *' className='bg-gray-100 outline-none text-sm flex-1 py-1 w-full' />
                        {errors.name && <p className='text-[13px] text-red-600'>name is required.</p>}
                    </div>


                    <div className='bg-gray-100 w-full p-2 flex items-start flex-col mb-5'>
                        <input type="text" {...register('email', { required: true, pattern: /^[A-Za-z0-9._-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/ })}  id="email"  onChange={handleChange} placeholder='Email *' className='bg-gray-100 outline-none text-sm flex-1 py-1 w-full' />
                        {errors.email && <p className='text-[13px] text-red-600'>Email is required.</p>}
                    </div>
                    <div className='bg-gray-100 w-full p-2 flex items-start flex-col mb-5'>
                        <input type="text"{...register('phone', { required: true, maxLength: 10, pattern: /^[0-9]{10}$/ })}  id="phone" placeholder='Phone no'  onChange={handleChange} className='bg-gray-100 outline-none text-sm flex-1 py-1 w-full' />
